    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                num_gen_batches += 1
                # Pop keys not required for the generation step
                ...
                # Check if we are at the last training step
                is_last_step = self.global_steps >= self.total_training_steps

                with _timer('step', timing_raw):
                    # generate a batch
                    with _timer('gen', timing_raw):
                        gen_batch_output = self.actor_rollout_wg.generate_sequences(gen_batch)   # worker group
                    ...
                    if not self.config.algorithm.filter_groups.enable:
                        ...
                        prompt_uid2metric_std = {uid: np.std(metric_vals) for uid, metric_vals in prompt_uid2metric_vals.items()}

                        # Filter based on metric standard deviation (std > 0)
                        kept_prompt_uids = [uid for uid, std in prompt_uid2metric_std.items() if std > 0 or len(prompt_uid2metric_vals[uid]) == 1]
                        num_prompt_in_batch += len(kept_prompt_uids)
                        kept_traj_idxs = [idx for idx, traj_from_prompt_uid in enumerate(new_batch.non_tensor_batch['uid']) if traj_from_prompt_uid in kept_prompt_uids]
                        ...
                        if num_prompt_in_batch < prompt_bsz:
                            ...
                            # Align the batch
                            traj_bsz = self.config.data.train_batch_size * self.config.actor_rollout_ref.rollout.n  # bs * n_rollout
                ...
                # Collect additional metrics for logging
