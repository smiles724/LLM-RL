...
def compute_data_metrics(batch, use_critic=True):
    ...
    metrics = {  # score
        ...
        'critic/rewards/min': torch.min(sequence_reward).detach().item(),  # adv
        'critic/advantages/mean': torch.mean(valid_adv).detach().item(), 'critic/advantages/max': torch.max(valid_adv).detach().item(),
        'critic/advantages/min': torch.min(valid_adv).detach().item(),  # returns
        'critic/returns/mean': torch.mean(valid_returns).detach().item(), 'critic/returns/max': torch.max(valid_returns).detach().item(),
        'critic/returns/min': torch.min(valid_returns).detach().item(), **({  # values
                                                                               'critic/values/mean': torch.mean(valid_values).detach().item(),
                                                                               'critic/values/max': torch.max(valid_values).detach().item(),
                                                                               'critic/values/min': torch.min(valid_values).detach().item(),  # vf explained var
                                                                               'critic/vf_explained_var': (
                                                                                       1.0 - return_diff_var / (return_var + 1e-5)).detach().item(), } if use_critic else {}),
...
def compute_timing_metrics(batch, timing_raw):
    ...
    num_tokens_of_section = {'gen': num_response_tokens, **{name: num_overall_tokens for name in ['ref', 'values', 'adv', 'update_critic', 'update_actor']}, }
    ...
    def fit(self):
        ...
        for _ in range(self.config.trainer.total_epochs):
            ...
            for batch_dict in self.train_dataloader:
                ...
                # pop those keys for generation (uuid.uuid4() to make a new 128-bit random UUID e.g., "3f5b8a2c-9e6a-4c2e-b7d1-8d6f0f2a1c3e")
                ...
                with _timer('step', timing_raw):
                    ...
                    if self.hint and valid_mask_with_hint.any():
                        ...
                        hint_batch = self.actor_rollout_wg.generate_sequences(hint_batch)  # generate a response with hint! (B)  -> (B * G)
                        if self.use_rm:  # no reward model used
                    ...
                    if self.config.trainer.save_freq > 0 and self.global_steps % self.config.trainer.save_freq == 0:
                        with _timer('save_checkpoint', timing_raw):
                            self._save_checkpoint()   # save locally
