...
def get_kl_controller(config):
    if config.critic.kl_ctrl.type == 'fixed':
        ...
        kl_ctrl = AdaptiveKLController(init_kl_coef=config.critic.kl_ctrl.kl_coef, target_kl=config.critic.kl_ctrl.target_kl, horizon=config.critic.kl_ctrl.horizon)
    else:
...
def compute_grpo_outcome_advantage(token_level_rewards: torch.Tensor, eos_mask: torch.Tensor, index: torch.Tensor, epsilon: float = 1e-6, mask_truncated_samples: bool = False):
    """
