...
class vLLMRollout(BaseRollout):
    ...
    def __init__(self, actor_module: nn.Module, config: DictConfig, tokenizer, model_hf_config, reward_fn, val_reward_fn, **kwargs):
        ...
        self.inference_engine = LLM(actor_module, tokenizer=tokenizer, model_hf_config=model_hf_config, tensor_parallel_size=tensor_parallel_size, dtype=config.dtype,
                                    enforce_eager=config.enforce_eager, gpu_memory_utilization=config.gpu_memory_utilization, skip_tokenizer_init=False,
                                    max_model_len=config.prompt_length + config.response_length, max_num_batched_tokens=max_num_batched_tokens,
                                    enable_chunked_prefill=config.enable_chunked_prefill, load_format=config.load_format)
        ...
        kwargs = dict(n=1, logprobs=1,  # can be set to 0 and let actor to recompute
    ...
    @torch.no_grad()
    def generate(self, prompts, idx, attention_mask, position_ids, **kwargs):
        ...
        if not do_sample:
            kwargs = {'best_of': 1, 'top_p': 0.95, 'top_k': -1, 'min_p': 0.0, 'temperature': 0, 'n': 1}
        ...
        kwargs['n'] = 1   # todo: only sample 1 response? no, just reset the parameter
        if do_sample:   # copy self.config.n for each prompt
            idx_list = [deepcopy(item) for item in idx_list for _ in range(self.config.n)]

        # Generate sequences
        with self.update_sampling_params(**kwargs):
            output = self.inference_engine.generate(prompts=None, sampling_params=self.sampling_params, prompt_token_ids=idx_list, use_tqdm=False)
        ...
        batch = TensorDict({'prompts': idx, 'responses': response, 'input_ids': seq, 'attention_mask': attention_mask, 'position_ids': position_ids}, batch_size=batch_size)
    ...
    def generate_sequences(self, prompts: DataProto, hint: bool = False, **kwargs) -> DataProto:
        ...
        batch, non_tensor_batch, is_validation = self.generate(prompts, idx, attention_mask, position_ids, **kwargs)
