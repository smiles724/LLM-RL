...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(self, micro_batch, temperature) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                output = self.actor_module(input_ids=input_ids_rmpad, attention_mask=None, position_ids=position_ids_rmpad,
                ...
                output = self.actor_module(input_ids=input_ids, attention_mask=attention_mask, position_ids=position_ids, use_cache=False)  # prevent model thinks we are generating
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    data = {'actor/entropy_loss': entropy_loss.detach().item(), 'actor/pg_loss': pg_loss.detach().item(), 'actor/pg_clipfrac': pg_clipfrac.detach().item(),
    ...
    def compute_sft(self, data: DataProto, ):
        ...
        assert self.config.ppo_mini_batch_size % self.config.ppo_micro_batch_size == 0   # 256 % 64
        self.gradient_accumulation = self.config.ppo_mini_batch_size // self.config.ppo_micro_batch_size
        ...
        metrics = {}
        for _ in range(self.config.ppo_epochs):   # todo: rename parameters
            for batch_idx, data in enumerate(dataloader):
                # split batch into micro_batches
                mini_batch = data
                if self.config.use_dynamic_bsz:
                ...
                for data in micro_batches:
                    ...
                    outputs = self.actor_module(input_ids=data['input_ids'], attention_mask=data['attention_mask'], position_ids=data['position_ids'], labels=data['responses'],
                                                use_cache=False)
                    sft_loss = outputs.loss
                    loss = sft_loss / len(micro_batches)   # todo: self.gradient_accumulation?   # todo: batch size scaling after filtering?
                    loss.backward()
